...
import { Temp60Post } from "./database/temp60Post.model.js";
...
async function run() {
  ...
  // 4. Map mỗi dòng thành document Temp60Post
  const docs = rows
    .map(r => {
  ...
  for (let i = 0; i < docs.length; i += BATCH_SIZE) {
    ...
    try {
      const res = await Temp60Post.insertMany(batch, { ordered: false });
  ...
  console.log(`🎉 Import completed (${docs.length} Temp60Post items).`);
