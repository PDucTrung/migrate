...
import { TempNot60Post } from "./database/tempNot60Post.model.js";
...
async function run() {
  ...
  // 4. Map mỗi dòng thành document TempNot60Post
  const docs = rows
    .map(r => {
      return { id: r.id.toString().trim() };
    })
    .filter(d => d);
  ...
  for (let i = 0; i < docs.length; i += BATCH_SIZE) {
    ...
    try {
      const res = await TempNot60Post.insertMany(batch, { ordered: false });
  ...
  console.log(`🎉 Import completed (${docs.length} TempNot60Post items).`);
